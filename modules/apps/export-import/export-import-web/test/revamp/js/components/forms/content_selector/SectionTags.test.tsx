...
import SectionTags from '../../../../../../src/main/resources/META-INF/resources/revamp/js/components/forms/content_selector/SectionTags';

describe('SectionTags', () => {
	it('renders nothing when both counts are zero or undefined', () => {
		const {container: emptyContainer} = render(<SectionTags />);

		expect(emptyContainer).toBeEmptyDOMElement();

		const {container: zeroContainer} = render(
			<SectionTags additionCount={0} deletionCount={0} />
		);
	...
	it('renders only the items text when there are additions but no deletions', () => {
		render(<SectionTags additionCount={5} />);
	...
	it('renders only the deletions badge when there are deletions but no additions', () => {
		render(<SectionTags deletionCount={3} />);
	...
	it('renders both indicators when both counts are positive', () => {
		render(<SectionTags additionCount={5} deletionCount={3} />);
